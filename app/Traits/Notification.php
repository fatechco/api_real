    ...
    ): void
    {
        ...
        dispatch(function () use ($model, $receivers, $message, $title, $data, $userIds) {
            ...
            foreach ($receivers as $receiver) {
                ...
                if ($r->status() !== 200) {
                    User::whereJsonContains('firebase_token', $receiver)
                        ->update([
                            'firebase_token' => DB::raw("JSON_REMOVE(firebase_token, JSON_UNQUOTE(JSON_SEARCH(firebase_token, 'one', '$receiver')))"),
                        ]);
                }
//                Log::error('fcm', $r->json());
            }
    ...
    ): void
    {
        ...
        User::select([
            ...
            ->orderBy('id')
            ->chunk(100, function ($users) use ($model, $data, $locale) {
    ...
    public function getReplace(Booking $booking): array
    {
        ...
        return [
            ...
            'master'  => $booking->master?->full_name ?? 'no name',
        ];
    }
    ...
    {
        $this->bookingUserMaster($bookings, 'user');
    ...
    {
        //master notify
        [$message, $replace, $firebaseToken] = $this->getAttributes($model, $model->master_id, $key, $data);

        $this->sendNotification($model, $firebaseToken, "$message:$text", "$message:$text", $replace, [$model->master_id]);
        ...
        if ($notification?->notification?->active) {
            ...
            [$message, $replace, $firebaseToken] = $this->getAttributes($model, $model->user_id, $key, $data);

            $this->sendNotification($model, $firebaseToken, $message, $message, $replace, [$model->user_id]);

        }

        //seller notify
        [$message, $replace, $firebaseToken] = $this->getAttributes($model, $model->shop?->user_id, $key, $data);

        $this->sendNotification($model, $firebaseToken, $message, $message, $replace, [$model->shop?->user_id]);
    }
    ...
    public function getAttributes(Booking $model, int $id, string $key, array $data): array
    {
        ...
        [$editor, $lang, $firebaseToken] = match($id) {
            (int)$master?->id => [$master?->full_name, $master?->lang ?? $this->language, $master->firebase_token ?? []],
            (int)$user?->id   => [$user?->full_name,   $user?->lang   ?? $this->language, $user->firebase_token   ?? []],
            (int)$seller?->id => [$seller?->full_name, $seller?->lang ?? $this->language, $seller->firebase_token ?? []],
            default           => [$auth->full_name,    $auth->lang    ?? $this->language, $auth->firebase_token   ?? []],
        };
        ...
        return [$message, $replace, $firebaseToken];
    }
}
