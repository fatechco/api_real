...
class BaseService extends CoreService
{
    ...
    {
        try {
            $paymentProcess = PaymentProcess::with(['model', 'user'])->find($token);

            if (empty($paymentProcess)) {
                $paymentProcess = PaymentProcess::with(['model', 'user'])->find($secondToken);
            }
            ...
            if (isset($paymentProcess->data['status']) && $paymentProcess->data['status'] === $status) {
                return [
                    'status'  => false,
    ...
    public function getPayload(array $data, array $payload): array
    {
        ...
        } else if (data_get($data, 'auction_id')) {

            $key = 'auction_id';
            $before = $this->beforeAuction($data, $payload);
    ...
    public function beforeBooking(array $data, array|null $payload): array
    {
        ...
        $totalPrice = ceil($booking->rate_total_price + $booking->children?->sum('rate_total_price'));
    ...
    public function beforeMemberShip(array $data, array|null $payload): array
    {
        ...
        $totalPrice  = ceil($memberShip->price);
    ...
    public function beforeParcel(array $data, array|null $payload): array
    {
        ...
        $totalPrice  = round($parcel->rate_total_price, 1);
