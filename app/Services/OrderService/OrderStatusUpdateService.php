...
class OrderStatusUpdateService extends CoreService
{
        ...
        try {
            ...
            $order = DB::transaction(function () use ($order, $status) {
                ...
                if ($status == Order::STATUS_DELIVERED) {
                    ...
                    Utility::topUpCashBack($point, $order, $this->language);
     ...
     * @throws Exception
     */
    public function updateNotes(Order $order, array $data): void
    {
        try {
     ...
     */
    private function refund($order): void
    {
        if ($order->transaction->paymentSystem->tag != Payment::TAG_CASH) {
            $this->paymentRefund($order);
        }
