            ...
            foreach ($master->disabledTimes as $disabledTime) {
                ...
                if (in_array($disabledTime->repeats, $repeatTypes)) {
                    ...
                    if (!$created && isset($data[$disabledTimeFormat]['disabled_times'][0])) {
                        ...
                        $startTime = new DateTime(reset($disabledTimes));
...
//            }
    ...
    private function collectBookingDays(array $data, User $master): array
    {
        ...
        foreach ($master->masterBookings as $masterBooking) {
            ...
            [$startDate, $startTime] = explode(' ', date('Y-m-d H:i', strtotime($masterBooking->start_date)));
            [$endDate,   $endTime]   = explode(' ', date('Y-m-d H:i', strtotime($masterBooking->end_date)));

            $startTime = new DateTime($startTime);
            $endTime   = new DateTime($endTime);
            $skipMinute = $masterBooking->serviceMaster->interval + $masterBooking->serviceMaster->pause;
    ...
    public function mergeTimes(array $data, ?array $disabledTimes): array
    {
        ...
        $data['disabled_times'] = $disabledTimes->unique()->sort()->values()->toArray();
