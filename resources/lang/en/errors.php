...
return [
    ...
    */
    ...
    $e::ADD_BOOKING_CASHBACK  => $languages['ADD_BOOKING_CASHBACK']  ?? 'Added booking cashback',
    ...
    $e::STATUS                          => $languages['status'] ?? 'Status',
    $e::SERVICE_TYPE                    => $languages['type'] ?? 'Service place',
    $e::SHOP                            => $languages['shop'] ?? 'Shop',
    $e::MASTER_FULL_NAME                => $languages['master.fullname'] ?? 'Master fullname',
    $e::SERVICE_NAME                    => $languages['service.name'] ?? 'Service name',
    $e::DATE_FROM                       => $languages['date.from'] ?? 'Date from',
    $e::DATE_TO                         => $languages['date.to'] ?? 'Date to',
    $e::CLIENT                          => $languages['client'] ?? 'Client',
    $e::PAYMENT_TYPE                    => $languages['payment.type'] ?? 'Payment type',
    $e::TRANSACTION_STATUS              => $languages['transaction.status'] ?? 'Transaction status',
    ...
    $e::SERVICE_FEE                     => $languages['service.fee'] ?? 'Service fee',
    $e::SERVICE_MASTER_PRICE            => $languages['service.master.price'] ?? 'Service master price',
    $e::TOTAL_PRICE                     => $languages['total.price'] ?? 'Total price',
    $e::DELIVERY_FEE                    => $languages['delivery.fee'] ?? 'Delivery fee',
    $e::ADDRESS_PLACE                   => $languages['address.place'] ?? 'Address place',
    $e::PRICE_WITHOUT_TAX               => $languages['price.without.tax'] ?? 'Price without tax',
    $e::DELIVERY_DATE_TIME              => $languages['delivery.date.time'] ?? 'Delivery date time',
    $e::TOTAL_PRICE_WITHOUT_TAX         => $languages['total_price.without.tax'] ?? 'Total price without tax',
    $e::SEND_GIFT_CART                  => $languages['send.gift.cart'] ?? ':sender sent you a gift cart',
    $e::GIFT_CART_PRICE                 => $languages['gift.cart.price'] ?? 'Gift cart price',
    $e::EXTRA_PRICE                     => $languages['extra.price'] ?? 'Extra price',
    $e::BOOKING                         => $languages['booking'] ?? 'Booking',
    $e::GENDER                          => $languages['gender'] ?? 'Gender',
    $e::ALL_GENDER                      => $languages['all.gender'] ?? 'All gender',
    $e::MALE                            => $languages['male'] ?? 'Male',
    $e::FEMALE                          => $languages['female'] ?? 'Female',
    $e::MEMBERSHIP                      => $languages['membership'] ?? 'Membership',
    $e::TAX                             => $languages['TAX'] ?? 'Tax',
    $e::DATE                            => $languages['DATE'] ?? 'Date',

    $e::MAKE_DEPOSIT                    => $languages['MAKE_DEPOSIT']       ?? 'Make a deposit',
    $e::MIN_AUCTION_PRICE               => $languages['MIN_AUCTION_PRICE']  ?? 'price must be more than min price',
    $e::LAST_AUCTION_BID                => $languages['LAST_AUCTION_BID']   ?? 'price must be more than last bid',
    $e::AUCTION_END                     => $languages['AUCTION_END']        ?? 'auction ended',
    $e::AUCTION_NOT_ACCEPTED            => $languages['AUCTION_NOT_ACCEPTED'] ?? 'auction is not accepted',
];
